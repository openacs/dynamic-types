...
begin
  ...
  for type_rec in select ot2.object_type, tree_level(ot2.tree_sortkey) as level
                  ...
                  order by level desc
  loop
    if not (type_rec.object_type = ''acs_object'') and 
       not (type_rec.object_type = ''content_revision'') then
...
begin
  ...
  -- TODO: add the columns below for acs_objects when this is rolled 
  -- forward to 5.2
